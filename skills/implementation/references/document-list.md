...
| IMP | IMP-001〜IMP-004 | 実装報告・マイグレーション。IMP-003は初回構築・大規模環境変更時のみ |
| UT | UT-001〜UT-003 | このプロジェクトのテスト結果・不具合 |
...
>
> **作成単位**: IMP-001・IMP-002は機能別、IMP-003・IMP-004はプロジェクト共通。
...
> **配置先**: すべて `projects/PRJ-{NNN}/UT/`（プロジェクト固有）。
>
> **作成単位**: UT-001・UT-002は機能別、UT-003はプロジェクト内の全機能不具合を集約したプロジェクト共通。
