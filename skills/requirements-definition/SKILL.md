...
description: |
  ...
  **3フェーズ構成**:
  1. ヒアリング（Plan Mode）: ファイル作成なし・質問のみ
  2. ドキュメント作成方針の確認: ヒアリング結果を整理してユーザーに承認を求める
  3. ドキュメント生成: 承認後に REQ-001〜REQ-008 を作成
...
DDD の概念を活用し、ビジネス担当者へのヒアリングを3フェーズで進め、要件定義書（REQ-001〜REQ-008）を作成する。
...
> **フェーズの制約**: このフェーズでは**ファイルを一切作成しない**。質問と情報収集のみを行う。

#### ステップ 1-1: プロジェクト種別の確認
...
1. **ヒアリング結果のサマリー**を作成する
   - プロジェクト概要（システム名・目的・スコープ・制約）
   - 特定したドメイン（コンテキスト・エンティティ・ビジネスルール）
   - ユースケース・機能サマリー（アクター・件数）
   - 収集した用語数

2. **ドキュメント作成方針表**を作成する
   - REQ-001〜REQ-008 の作成要否と主な内容を表形式で示す
   - 改修プロジェクトは変更対象のドキュメントのみ「要」にする
   - 外部連携がなければ REQ-007 は「不要」とする

3. **ExitPlanMode ツール**を呼び出してユーザーに承認を求める
   - ユーザーが承認 → フェーズ 3 に進む
   - 修正が必要 → ヒアリングに戻るか方針を調整して再提示する

---

### フェーズ 3: ドキュメント生成

`references/document-templates.md` のテンプレートを使い、承認された方針書に従い REQ ドキュメントを Markdown ファイルとして出力する。
